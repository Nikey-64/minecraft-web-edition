...
Player.prototype.setInputCanvas = function( id )
{
	...
	canvas.onmousedown = function( e ) { t.onMouseEvent( e.clientX, e.clientY, MOUSE.DOWN, e.which == 3, e ); return false; }
	canvas.onmouseup = function( e ) { t.onMouseEvent( e.clientX, e.clientY, MOUSE.UP, e.which == 3, e ); return false; }
	canvas.onmousemove = function( e ) { t.onMouseEvent( e.clientX, e.clientY, MOUSE.MOVE, e.which == 3, e ); return false; }
	...
	// Detectar cuando el pointer lock cambia
	document.onpointerlockchange = function() {
		t.pointerLocked = (document.pointerLockElement === canvas || document.mozPointerLockElement === canvas);
		if ( t.pointerLocked ) {
			// Establecer posición inicial del ratón al centro
			t.centerX = canvas.width / 2;
			t.centerY = canvas.height / 2;
		}
	}
	document.onmozpointerlockchange = function() {
		t.pointerLocked = (document.pointerLockElement === canvas || document.mozPointerLockElement === canvas);
		if ( t.pointerLocked ) {
			// Establecer posición inicial del ratón al centro
			t.centerX = canvas.width / 2;
...
// onMouseEvent( x, y, type, rmb, event )
//
// Hook for mouse input.

Player.prototype.onMouseEvent = function( x, y, type, rmb, event )
{
	if ( type == MOUSE.DOWN ) {
	...
	} else if ( type == MOUSE.MOVE && event && this.pointerLocked ) {
		// Solo actualizar la cámara si el pointer está bloqueado
		var movementX = event.movementX || event.mozMovementX || 0;
		var movementY = event.movementY || event.mozMovementY || 0;

		this.targetPitch = this.angles[0] - movementY / 200;
		this.targetYaw = this.angles[1] + movementX / 200;
...
Player.prototype.doBlockAction = function( x, y, destroy )
{
	...
	if ( block != false )
	{
		...
		// Mark world as having unsaved changes and remember last modified position
		if ( this.world ) {
			this.world.hasUnsavedChanges = true;
			if ( destroy ) {
				this.world.lastModifiedPos = { x: block.x, y: block.y, z: block.z };
			} else {
				this.world.lastModifiedPos = {
					x: block.x + block.n.x,
					y: block.y + block.n.y,
					z: block.z + block.n.z
				};
