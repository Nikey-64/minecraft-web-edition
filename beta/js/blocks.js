...
{
	// Use chunk-based world access instead of world.blocks
	var block = world.getBlock( x, y, z );
	if ( !block || block === BLOCK.AIR ) return;
	
	var blockLit = z >= lightmap[x][y];
	
	// Neighbour blocks (world.getBlock() safely returns AIR out of bounds)
	var above  = world.getBlock( x,     y,     z + 1 );
	var below  = world.getBlock( x,     y,     z - 1 );
	var front  = world.getBlock( x,     y - 1, z     );
	var back   = world.getBlock( x,     y + 1, z     );
	var left   = world.getBlock( x - 1, y,     z     );
	var right  = world.getBlock( x + 1, y,     z     );
	
	var bH = block.fluid && ( z == world.sz - 1 || !above.fluid ) ? 0.9 : 1.0;
	
	// Top
	if ( z == world.sz - 1 || above.transparent || block.fluid )
	{
	...
	// Back
	if ( y == world.sy - 1 || back.transparent )
	{
	...
	// Right
	if ( x == world.sx - 1 || right.transparent )
	{
