...
var clientInfo = {
	...
	detect: function() {
		...
		// Detect low-end devices (older mobile, low RAM)
		this.isLowEnd = this.isMobile && (
			/Android [2-6]/i.test(navigator.userAgent) ||
			/iPhone OS [1-9]/i.test(navigator.userAgent) ||
			navigator.deviceMemory && navigator.deviceMemory < 4
		);

		// Detect high precision support
		var canvas = document.createElement('canvas');
		var gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl');
		if (gl) {
			var ext = gl.getExtension('OES_vertex_array_object');
			this.supportsHighP = !!ext;
			this.maxTextureSize = gl.getParameter(gl.MAX_TEXTURE_SIZE);
		}

		console.log('Client detection:', this);
	}
};
...
// Shaders - optimized based on client capabilities
var vertexSource, fragmentSource;

if (clientInfo.isLowEnd) {
	// Simplified shaders for low-end devices (no fog)
	vertexSource =
	...
	fragmentSource =
		...
		"varying vec2 vTexCoord;"+
		"void main() {"+
		"	vec4 color = texture2D( uSampler, vTexCoord ) * vColor;"+
		"	if ( color.a < 0.1 ) discard;"+
		"	gl_FragColor = color;"+
		"}";
} else {
	// Full shaders for capable devices (with fog support)
	vertexSource =
		"uniform mat4 uProjMatrix;"+
		"uniform mat4 uViewMatrix;"+
		"uniform mat4 uModelMatrix;"+
		...
		"	vColor = aColor;"+
		"	vTexCoord = aTexCoord;"+
		"	vFogDistance = length(worldPos.xyz);"+
		"}";
	fragmentSource =
...
function Renderer( id )
{
	...
	try
	{
		gl = this.gl = canvas.getContext( "webgl" ) || canvas.getContext( "experimental-webgl" );
	} catch ( e ) {
	...
	// Set default render distance based on client capabilities
	var defaultRenderDistance = clientInfo.isLowEnd ? 1 : (clientInfo.isMobile ? 2 : 8);
	this.renderDistance = defaultRenderDistance;
	...
	// Optimization: Cache for chunk distance calculations
	this.lastChunkUpdatePos = null;
	this.chunkUpdateInterval = 10; // Update chunks every N frames
	this.chunkUpdateCounter = 0;
	
	// Optimization: Pre-calculate render distance squared to avoid sqrt
	this.renderDistanceSquared = 0;
	// Will be updated when setWorld is called (needs chunkSize)
}
...
Renderer.prototype.draw = function()
{
	...
	if (clientInfo.isLowEnd) {
		// Skip frames for low-end devices to maintain performance
	...
	// Draw level chunks (optimized: only iterate active chunks)
	...
	if ( chunks != null )
	{
		// Optimized: cache chunks.length to avoid repeated property access
		var chunksLength = chunks.length;
		for ( i = 0; i < chunksLength; i++ )
		{
			chunk = chunks[i];
			// Only check active and buffer in one condition
			if ( chunk.active && chunk.buffer )
			{
	...
	// Update active chunks based on player position (throttled for performance)
	this.chunkUpdateCounter++;
	if ( this.chunkUpdateCounter >= this.chunkUpdateInterval || !this.lastChunkUpdatePos )
	{
	...
	// Optimized: cache reference and check if players exist
	if ( players )
	{
		...
		// Optimized: declare aniangle variable
		var aniangle = 0;
...
Renderer.prototype.toggleFPS = function()
{
...
Renderer.prototype.buildPlayerName = function( nickname )
{
	...
	// Ensure we have a safe string nickname
...
Renderer.prototype.setWorld = function( world, chunkSize )
{
	...
	// Update render distance squared now that we have chunkSize
	this.updateRenderDistanceSquared();
	
	// Initially activate chunks around spawn point
	if ( world.spawnPoint ) {
		this.activateChunksAroundPosition(world.spawnPoint);
	}
}
...
Renderer.prototype.onBlockChanged = function( x, y, z )
{
	...
	var chunk, i, chunkLength = chunks.length;
	var start, end;

	for ( i = 0; i < chunkLength; i++ )
	{
		chunk = chunks[i];
		// Only mark dirty if chunk is active
		if ( !chunk.active ) continue;
		
		start = chunk.start;
		end = chunk.end;

		// Optimization: Early exit conditions and cached bounds
		...
		if ( x >= start[0] && x < end[0] && y >= start[1] && y < end[1] && z >= start[2] && z < end[2] )
		{
			chunk.dirty = true;
		}
		else if ( x >= start[0] && x < end[0] && y >= start[1] && y < end[1] && ( z >= end[2] || z == start[2] - 1 ) )
		{
			chunk.dirty = true;
		}
		else if ( x >= start[0] && x < end[0] && z >= start[2] && z < end[2] && ( y == end[1] || y == start[1] - 1 ) )
		{
			chunk.dirty = true;
		}
		else if ( y >= start[1] && y < end[1] && z >= start[2] && z < end[2] && ( x == end[0] || x == start[0] - 1 ) )
		{
			chunk.dirty = true;
		}
	}
...
Renderer.prototype.buildChunks = function( count )
{
	...
	if ( !chunks || !world ) return;

	// Optimization: Collect dirty chunks and sort by distance to prioritize nearby chunks
	var dirtyChunks = [];
	var i, chunk, chunkLength = chunks.length;
	...
	for ( i = 0; i < chunkLength; i++ )
	{
		...
		if ( chunk.active && chunk.dirty )
		{
			...
			if ( camPos ) {
				// Calculate distance to prioritize nearby chunks
				var chunkCenterX = (chunk.start[0] + chunk.end[0]) * 0.5;
				var chunkCenterY = (chunk.start[1] + chunk.end[1]) * 0.5;
				var chunkCenterZ = (chunk.start[2] + chunk.end[2]) * 0.5;
				var dx = chunkCenterX - camPos[0];
				var dy = chunkCenterY - camPos[1];
				var dz = chunkCenterZ - camPos[2];
				distanceSquared = dx * dx + dy * dy + dz * dz;
			}
			
			dirtyChunks.push( { chunk: chunk, distanceSquared: distanceSquared } );
		}
	}
	
	// Sort by distance (closest first) for better perceived performance
	dirtyChunks.sort( function( a, b ) {
		return a.distanceSquared - b.distanceSquared;
	});
	
	// Build chunks starting with the closest
	var built = 0;
	for ( i = 0; i < dirtyChunks.length && built < count; i++ )
	{
		chunk = dirtyChunks[i].chunk;
		
		var vertices = [];
		var start = chunk.start;
		var end = chunk.end;

		// Optimization: Cache world properties
		var worldSz = world.sz;
		var getBlock = world.getBlock.bind(world);
		
		// Ensure chunks are loaded before building
		// Get chunk coordinates for this render chunk
		var renderChunkCoords = {
			cx: Math.floor( start[0] / world.chunkSize ),
			cy: Math.floor( start[1] / world.chunkSize ),
			cz: Math.floor( start[2] / world.chunkSize )
		};
		
		// Load required chunks (synchronously for now)
		var maxCx = Math.floor( (end[0] - 1) / world.chunkSize );
		var maxCy = Math.floor( (end[1] - 1) / world.chunkSize );
		var maxCz = Math.floor( (end[2] - 1) / world.chunkSize );
		
		for ( var cx = renderChunkCoords.cx; cx <= maxCx; cx++ ) {
			for ( var cy = renderChunkCoords.cy; cy <= maxCy; cy++ ) {
				for ( var cz = renderChunkCoords.cz; cz <= maxCz; cz++ ) {
					// Load chunk synchronously if not already loaded
					if ( !world.loadedChunks[world.getChunkKey(cx, cy, cz)] ) {
						world.loadChunkSync( cx, cy, cz );
					}
				}
			}
		}
		...
		for ( var x = start[0] - 1; x < end[0] + 1; x++ )
		{
			...
			{
				for ( var z = worldSz - 1; z >= 0; z-- )
				{
					lightmap[x][y] = z;
					if ( !getBlock( x, y, z ).transparent ) break;
				}
			}
		}
...
// activateChunksAroundPosition( position )
//
// Activates chunks around a specific position.

Renderer.prototype.activateChunksAroundPosition = function( position )
{
	var chunks = this.chunks;
	var pos = position.toArray ? position.toArray() : position;
	
	if ( !pos || !chunks ) return;

	// Optimization: Use squared distance
	var maxDistanceSquared = ((this.renderDistance + 1) * this.chunkSize) * ((this.renderDistance + 1) * this.chunkSize);
	var px = pos[0];
	var py = pos[1];
	var pz = pos[2];
	var i, chunk, chunkLength = chunks.length;
	...
	for ( i = 0; i < chunkLength; i++ )
	{
		...
		// Optimization: Pre-calculate chunk center
		var chunkCenterX = (chunk.start[0] + chunk.end[0]) * 0.5;
		var chunkCenterY = (chunk.start[1] + chunk.end[1]) * 0.5;
		var chunkCenterZ = (chunk.start[2] + chunk.end[2]) * 0.5;

		// Optimization: Calculate squared distance (no sqrt)
		var dx = chunkCenterX - px;
		var dy = chunkCenterY - py;
		var dz = chunkCenterZ - pz;
		var distanceSquared = dx * dx + dy * dy + dz * dz;

		// Check if chunk should be active (preload one extra layer)
		if ( distanceSquared <= maxDistanceSquared && !chunk.active )
		{
			// Activate chunk
			chunk.active = true;
			chunk.dirty = true;
...
// updateActiveChunks()
//
// Updates which chunks are active based on player position and render distance.

Renderer.prototype.updateActiveChunks = function()
{
	var chunks = this.chunks;
	var playerPos = this.camPos;
	
	if ( !playerPos || !chunks ) return;
	
	// Optimization: Use squared distance to avoid expensive sqrt
	var maxDistanceSquared = this.renderDistanceSquared;
	var chunkSize = this.chunkSize;
	var gl = this.gl;
	var chunk, i, chunkLength = chunks.length;
	
	// Optimization: Cache player position coordinates
	var px = playerPos[0];
	var py = playerPos[1];
	var pz = playerPos[2];
	...
	for ( i = 0; i < chunkLength; i++ )
	{
		...
		// Optimization: Pre-calculate chunk center (could be cached per chunk)
		var chunkCenterX = (chunk.start[0] + chunk.end[0]) * 0.5;
		var chunkCenterY = (chunk.start[1] + chunk.end[1]) * 0.5;
		var chunkCenterZ = (chunk.start[2] + chunk.end[2]) * 0.5;

		// Optimization: Calculate squared distance (no sqrt needed)
		var dx = chunkCenterX - px;
		var dy = chunkCenterY - py;
		var dz = chunkCenterZ - pz;
		var distanceSquared = dx * dx + dy * dy + dz * dz;
		...
		var shouldBeActive = distanceSquared <= maxDistanceSquared;
	...
	// Cache current position for throttling
	this.lastChunkUpdatePos = [px, py, pz];
}

// updateRenderDistanceSquared()
//
// Updates the squared render distance for optimization (avoids sqrt in comparisons)

Renderer.prototype.updateRenderDistanceSquared = function()
{
	if ( this.renderDistance && this.chunkSize ) {
		var maxDist = this.renderDistance * this.chunkSize;
		this.renderDistanceSquared = maxDist * maxDist;
	}
}

// setRenderDistance( distance )
//
// Sets the render distance and updates the squared value for optimization.
